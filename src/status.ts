 ...
 *
 * Run via: pnpm run status
 */

import chalk from "chalk";
import { format } from "date-fns";
import { pathExists } from "path-exists";
import { getBackupRoot, getIntervalDays } from "./config.js";
...
/* .env is optional here: before setup, the defaults still give a useful report. */
try {
...
async function main(): Promise<void> {
  ...
  if (!(await pathExists(".env"))) {
    log.step(chalk.yellow("Not configured yet"), "Run: pnpm run setup");
  }
...
main().catch((err: unknown) => {
  ...
  log.error(message);
  log.end(chalk.red("✖ Status failed"));
  process.exit(1);
});
